...
exports.getAccount = async (req, res) => {
  ...
  const accounts = JSON.parse(file).filter(v => v.onlyid === onlyid)
...
exports.createAccount = async (req, res) => {
  ...
  if (accounts.filter(v => v.onlyid === newAccount.onlyid).length != 0) {
    res.status(400).send()
  } else {
...
exports.updateAccount = async (req, res) => {
  ...
  const file = await asyncReadFile(req.app.locals.dataFilePath)
  const accounts = JSON.parse(file)
  const candidates = accounts.filter(v => v.onlyid === put.onlyid)
  if (candidates.length === 0) {
    this.createAccount(req, res)
  } else {
    accounts.forEach((value, index, array) => {
      if (value.onlyid === put.onlyid) {
        array[index] = {
          ...value,
          ...put
        }
...
exports.deleteAccount = async (req, res) => {
  ...
  const file = await asyncReadFile(req.app.locals.dataFilePath)
  const accounts = JSON.parse(file)
  const newAccounts = accounts.filter(v => v.onlyid !== onlyid)
